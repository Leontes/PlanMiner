...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        ...
        cout << "Sintaxis:\n";
        cout << "INSLV <semilla_problema>\n";
        cout << "       or\n";
        cout << "INSLV <semilla_problema> implicity\n\n";
        exit(1);
    }
	...
	if (argc > 4) {
	        E_Par_Completo=E.Extract_Training_Set(par, V.Consecuente(), atoi(argv[4]));
	}
	else {
	...
	int paso=100, pasoANT=0, trozo=0, inicio=0;
 ...
 //       tama[5]=paso+1; // La ultima posicion guarda el tamano real de elementos
	Indiv = rango[0]*10;
        multipoblacion G(NPobBin,NPobEnt,NPobRea,rango,rangoi,rangos,mut,cruce,
                         Elite,Indiv,tama,NFitness);
	...
	double *evo_media_training, *evo_media_test, *evo_umbral,
	       ...
	       *evo_media_1test, *evo_media_2test, *evo_media_3test, *evo_media_4test, *evo_media_5test,
	       *evo_global_test;
...
/*	  if ((it2 > 0) && (it2 % 10 == 0) && (it2 < 5000)) { // Paso variable
		pasoANT = paso;
	  	paso += 10;
//	  	paso += it2/10*2;
	  }*/
	  fin = inicio+paso;
 	  /*if (2*(trozo+paso)>limite){
	    E_Par = E_Par_Completo;
	    trozo = limite;
	    salir=true;
	  }
	  else {
	    E_Par = E_Par_Completo.SubSet(2*(trozo+paso));
	    trozo=2*(trozo+paso);
	  }*/
	  if (fin>=limite){
	    fin=limite;
	    salir=true;
	    //Subconjunto = E_Par_Completo.SubSet(inicio,limite);
	  }
	  //else {
	  ...
	  //if (inicio-3*paso<0)
	    //E_Anteriores = E_Par_Completo.SubSet(0,3*paso-1);
	  //else
	    //E_Anteriores = E_Par_Completo.SubSet(inicio-3*paso, inicio-1);


	  /*for (int k=0; k<E_Anteriores.N_Examples(); k++){
	    for (int l=0; l< E_Anteriores.N_Variables(); l++){
	      cout << "\t" << E_Anteriores.Data(k,l);
	    }
	    cout << endl;
	  }*/
	  cout << endl;
	  ...
	  int anterior_n;
	  // Calculo de la importancia de cada ejemplo por clase
	  E_Par.UnMarkAll();
	  ...
	  //cout << "Reglas acumuladas: \n";
	  //for (int l=0; l<R_prev[par].N_rule(); l++)
	    //cout << R_prev[par].SPrint(l) << endl;
	  //ruleset R_aux=R_prev[par];
	  //R[par]=R_prev[par];

	  //accuracy_old = Aciertos(R[par],V,E_Par,agregado,peso_agregado, fich_extension, F);

	  /*cout << "Aciertos del ultimo conjunto de reglas: " << endl;
	  cout << "\t sobre el acumulado de ejemplos anteriores: "<<
	           Aciertos(R_prev[par],V,E_Anteriores,agregado,peso_agregado, fich_extension, F) << endl;
	  cout << "\t sobre el nuevo conjunto de ejemplos: " << accuracy_old << endl << endl;

	  cout << "Aciertos del conjunto global de reglas aprendidas: " << endl;
	  cout << "\t sobre el acumulado de ejemplos anteriores: "<<
	           Aciertos(R_prev[par],V,E_Anteriores,agregado,peso_agregado, fich_extension, F) << endl;
	  cout << "\t sobre el nuevo conjunto de ejemplos: " <<
	           Aciertos(R_prev[par],V,E_Par,agregado,peso_agregado, fich_extension, F) << endl << endl;*/


	  //cin >> ch;

	  //if (cambio_en_reglas){
	   /* cout << "............................. Reajustando Reglas .......................\n";
	    R_prev[par].FixModifyToTrue();
	    ReajustarPesoaReglas(G,V,E_Par,R_prev[par],F, Tabla, Tabla_Nuevas);

	    R[par].FixModifyToTrue();
	    ReajustarPesoaReglas(G,V,E_Par,R_prev_dis[par],F, Tabla, Tabla_Nuevas);*/
	   ...
	   cout << "  Numero de Reglas: " << R_prev[par].N_rule() << "(" << R_prev[par].N_rule(0.5) << ")" << endl;
	    ...
	    //cambio_en_reglas=false;
	  //}

	  //accuracy_old = Aciertos(R_prev[par],V,E_Par,agregado,peso_agregado, fich_extension, F);
	  //umbral_acierto-=0.00;
	  ...
	  if (it2>=10 and it2%10==0){
	   //E_Anteriores = E_Par_Completo.SubSet(inicio-10*pasoANT, inicio-9*pasoANT-1);
	  // cout << "@" << it2-10 << " " << psubi[it2-10] << " " << psubf[it2-10] << endl;
	   ...
	   acumulado2+=evo_media_3training[it2-10];
	    //umbral_acierto = (1.0*it2)/(episodios)*(acumulado2/it2)+ ((1.0*episodios-it2)/(episodios))*umbral_acierto;
	   umbral_acierto = evo_media_3training[it2-10]-percent*evo_media_3training[it2-10];
	  }
	  else if (it2<10 and it2>1){
/// ","
	   //E_Anteriores = E_Par_Completo.SubSet(inicio-it2*pasoANT, inicio-(it2-1)*pasoANT-1);
	  ...
	  if (accuracy_old< (umbral_acierto)){
	    ...
	    //R[par]=R_prev[par];
	    //R_dis[par]=R_prev_dis[par];
	    //R_mix[par]=R_mix_prev[par];
	    ...
	    //Filtrar_Reglas_New(R[par],R_dis[par],R_mix[par], lista2, tama2, false);
	    genetcode *aux;
	    double nmas,nmenos;
	    int j=0;
	    ...
	    if (l_tama>=0 and l_tama<E_Par_Not_Covered.N_Examples() ){
	      ...
	      //E_Par_Not_Covered.Remove(l_aciertos,l_tama);
	  ...
	  //if (accuracy_old< (umbral_acierto/it)){
	  if (accuracy_old< (umbral_acierto)){
	    accuracy_old= Aciertos(R[par],V,E_Par_Not_Covered,F);
	    cout << "Aciertos de la descriptiva antes de lanzar el genetico: "
		    <<  accuracy_old << endl;
	    cambio_en_reglas=true;

	    //R[par].Anular_N_Mas_y_N_Menos_Todas();
	    //R_dis[par].Anular_N_Mas_y_N_Menos_Todas();
	    umbral_acierto-=0.00;
	    //R_mix[par].Anular_N_Mas_y_N_Menos_Todas();
	  ...
	  do
	  {
	      ...
	      //out.open(fich_extension.c_str(),ios::app);
	      //out << "Aciertos: " << accuracy_new[par]  << endl;
	      //out << "Numero de ejemplos: " << n <<endl;
	      //for (int i=0; i<rango[0]; i++)
		//  out << "Clase " << i <<": "<<ejemplos_por_clase[i]<<endl;
	      //out.close();

	      // Calculo de las medidas de informacion para el nivel de variable
	      //cout << "Calculando medidas de informacion\n";
	      //MedidasDeInformacion(V,E_Par, Med_Inform);
	      //MedidasDeInformacion_operandores(V,E_Par);
	      //cout << "Terminado el calculo de las medidas de informacion\n";
	      ...
	      //R_aux=R_prev[par];
	      //ruleset R_aux1 = R[par];
	      //R_aux.AddRuleset(R_aux1);
	      //accuracy_new[par] = Aciertos(R[par],V,E_Par,agregado,peso_agregado, fich_extension, F);
		  ...
		  (G.ValorFitness(0)>-1 and G.ValorFitness(0,1)>=1 ) or
		  (G.ValorFitness(0)<=-1 and G.ValorFitness(0,1)>=(-G.ValorFitness(0))+2 ) or
		  ( G.ValorFitness(0)==-E_Par.N_Examples()  and G.ValorFitness(0,1)>1) or
		  ((G.ValorFitness(0)==-E_Par.N_Examples()) and G.ValorFitness(0,1)==1 and G.ValorFitness(0,2)>0) or
		  ((G.ValorFitness(0)==-E_Par.N_Examples()) and G.ValorFitness(0,1)==1 and G.ValorFitness(0,2)==0 and G.ValorFitness(0,3)>0) or
		  ( (G.ValorFitness(0)==-E_Par.N_Examples()) and G.ValorFitness(0,1)==1 and G.ValorFitness(0,2)==0 and G.ValorFitness(0,3)==0 and G.ValorFitness(0,4)>0 ))*/
	      if (Added_Rule)
	      {
    		  int reglas_antes=R[par].N_rule();
    		  //cout << "--------------------------------- Filtrando Reglas\n";
    		  //cout << "\t\tSe eliminan las reglas (" << n_reglas_a_eliminar << "): ";
    		  //for (int i=0; i<n_reglas_a_eliminar; i++)
    		    //cout << " " << reglas_a_eliminar[i];
    		  //cout << endl;
    		  /*
    		  //cout << "Aciertos: " << accuracy_new[par]  << endl;
    		  //accuracy_new[par] = Filtrar_Reglas(R[par],V,E_Par,R_Descrip[par]);
    		  cout << "--------------------------------- Filtrando Reglas\n";
    		  Filtrar_Reglas(R[par],V,E_Par,F);
    		  //Filtrar_Reglas(R_Descrip[par],V,E_Par);
    		  accuracy_new[par] = Aciertos(R[par],V,E_Par,agregado, peso_agregado, fich_extension, F);
    		  ...
    		  /*int reglas_enteras=G.ValorFitness(0,1);
    		  if (reglas_antes-R[par].N_rule()!=reglas_enteras){
    		    sleep(4);
    		  }*/
	      }

	      /*if (iteracion>=tope and G.ValorFitness(0)>-1){ // Eliminar la regla 0
    		genetcode *aux;
    		double peso;
    		aux = R[par].Extract(0,peso);
    		G.PutCodigo(0,*aux);
    		delete aux;
    		accuracy_new[par] = Aciertos(R[par],V,E_Par,agregado, peso_agregado, fich_extension, F);
    		cout << "Eliminando la regla 0\n";
    		tope = iteracion+2*rango[0];
    		//sleep(1);
	      }*/
	      ...
	      //cout << "-------*\t\tTraining: "<< accuracy_new[par] << endl;

	      //cout << "Habia: " << anterior_n << " y ahora quedan: " << n << "                 Diferencia: " << anterior_n-n << endl;

	      /*if (Added_Rule and reducir_reglas){
    		cout << "=======> Desactivado el modo reducir reglas\n";
    		discriminante=true;
    		reducir_reglas=false;
	      }*/
	      ...
	      // lo mejor que he encontrado no es una regla buena, es decir -ne < G.ValorFitness(0) <= -1
	      // en este caso, busco si puedo mejorar o eliminar las reglas que ya tengo con reducir_reglas
	      if (G.ValorFitness(0)!=-2*E_Par_Not_Covered.N_Examples() and!reducir_reglas and !Added_Rule){
    		//cout << "=======> Activado el modo reducir reglas\n";
    		reducir_reglas=true;
    		discriminante=true;
    		Added_Rule=true;
	      }
	  ...
	  //umbral_acierto+= Aciertos(R[par],V,E_Par,F);
	  //umbral_acierto=0.99;
	  it++;
	  //while (!Terminado_Aprendizaje(G,E_Par,fase_final));

	  /*if (G.ValorFitness(0)>=1 or (G.ValorFitness(0)>=0 and G.ValorFitness(0,1)>1 ) )
	  {
	    ...
	    //if (R_prev[par].N_rule()==0){

	    if (it2>1){
	      if (it2>10){
    		//E_Anteriores = E_Par_Completo.SubSet(inicio-10*pasoANT, inicio-1);
    		E_Anteriores = E_Par_Completo.SubSet(psubi[it2-10], inicio-1);
	      }
	      else if (it2<=10){
    		//E_Anteriores = E_Par_Completo.SubSet(inicio-it2*pasoANT, inicio-1);
	...
	/*cout << "Creando la tabla de adaptaciones Ejemplos Anteriores.....\n";
	double ***Tabla1 = Crear_Tabla_Adaptaciones(V,E_Par_Completo);
	double ***Tabla_Nuevas1 = Crear_Tabla_Adaptaciones_Nuevas(F,E_Par_Completo);
	cout << "Tabla de adaptaciones creada!\n";

	cout << "Ajustando Peso Descriptivas...\n";
	ReajustarPesoaReglas(G,V,E_Par_Completo,R_prev[par],F, Tabla1, Tabla_Nuevas1);
	//cout << "Acierto Anterior (Descriptivas): " << Aciertos(R_prev[par],V,E_Anteriores,agregado,peso_agregado, fich_extension, F);
	cout << endl;

	cout << "Ajustando Peso Discriminantes...\n";
	ReajustarPesoaReglas(G,V,E_Par_Completo,R_prev_dis[par],F, Tabla1, Tabla_Nuevas1);
	//cout << "Acierto Anterior (Discriminantes): " << Aciertos(R_prev_dis[par],V,E_Anteriores,agregado,peso_agregado, fich_extension, F);

	//cout << endl;
	//cin >> ch;

	Destruir_Tabla_Adaptaciones(V,E_Par_Completo, Tabla1);
	Destruir_Tabla_Adaptaciones(F,E_Par_Completo, Tabla_Nuevas1);*/

        //if (ult_act_rules!= R_prev_dis[par].N_rule())
	  //AnalizarConjuntosDeReglas(V, R_prev_dis[par], R_prev[par], R_mix_prev[par]);

        // Termina de contar el tiempo de aprendizaje
        tiempo1=clock();


        test[par]=Aciertos(R_prev_dis[par],V,E_Par_Test,fich_extension, F);
	test_Descrip[par]=Aciertos(R_prev[par],V,E_Par_Test,fich_extension, F);
	...
	delete [] disparosR_Descrip;
        ...
        out.close();
        R_prev[par].SaveRuleInterpreted_append(fich_extension.c_str());
        out.open(fich_extension.c_str(),ios::app);
        out << "Predictivas: " << endl;
        out.close();
        R_prev_dis[par].SaveRuleInterpreted_append(fich_extension.c_str());
        out.open(fich_extension.c_str(),ios::app);
        out << "Patrones: " << endl;
