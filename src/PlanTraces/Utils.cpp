...
void to_table( std::string task, StatesLists states, std::vector < std::vector < std::vector < double > > > * datasets, std::vector < std::vector < std::pair<std::string, std::string> > > * attribLabelsVC){
  ...
  std::vector<double> nAttrib, otroVect;
    ...
    if((*attribLabels)[attr].second == "Numerical"){
      ...
      if(tieneAlgo == true){
        ...
        if(vectorDeZeros(nAttrib) == true){
          attribLabelsCP.push_back(std::pair <std::string, std::string> ("DELTA - " + (*attribLabels)[attr].first, "Numerical"));
          otroVect = nAttrib;
          otroVect.insert(otroVect.end(), nAttrib.begin(), nAttrib.end());
          datasetCP.push_back(otroVect);
        }
      }
    }
  }
...
void discoverInfo(std::vector < std::vector < double > > * dataset, std::vector < std::pair<std::string, std::string> > * attribLabels){
    ...
    std::vector < std::vector < double > > datasetCP;
    std::vector < std::pair<std::string, std::string> > attribLabelsCP;

    for (size_t i = 0; i < ((dataset -> size()) - 1); i++) {
      if (((*attribLabels)[i].first.find("DELTA") == std::string::npos)){
        datasetCP.push_back((*dataset)[i]);
        attribLabelsCP.push_back((*attribLabels)[i]);
      }
    }

    for (size_t i = 0; i < dataset -> size(); i++) {
      if (((*attribLabels)[i].first.find("DELTA") != std::string::npos)){
        listadeDiffs.push_back( std::pair<std::string, std::vector <double> > ((*attribLabels)[i].first.substr(8), std::vector <double>((*dataset)[i].begin(), (*dataset)[i].begin() + ((*dataset)[i].size()/2))));
      }
    }
    ...
    for (unsigned int i = 0; i < listadeDiffs.size(); i++){
      ...
      for(unsigned int attr = 0;attr < attribLabels->size(); attr++){
        ...
        if((((*attribLabels)[attr].first).find("DELTA") == std::string::npos) and ((*attribLabels)[attr].second == "Numerical") and (*attribLabels)[attr].first != "TIMESTAMP"){
          aux.clear();

          //Y los vamos metiendo en el dataset para el regresor
          for(unsigned int i = 0; i < ((*dataset)[0].size()/2); i++){
            aux.push_back((*dataset)[attr][i + ((*dataset)[0].size()/2)]);
          }
        ...
        if(esPrecondicionable((*attribLabels)[attr].first, listadeDiffs)){
          ...
          for(unsigned int attr2 = attr; attr2 < attribLabels->size(); attr2++){
            if((attr != attr2) and ((*attribLabels)[attr2].second == "Numerical") and ((*attribLabels)[attr2].first != "TIMESTAMP")){
              ...
              nAttrib.clear();
              tieneAlgo = false;
              for(unsigned int i = 0; i < (*dataset)[attr].size(); i++){
    ...
    //Y finiquitamos todo metiendo las etiquetas de las clases en el dataset
    std::vector<double> classVec((*dataset)[0].size(), 1.0);
    for(unsigned int i = 0; i < ((*dataset)[0].size()/2); i++){
      classVec[i] = 0.0;
    }
    datasetCP.push_back(classVec);
